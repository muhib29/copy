...
export default function CategoriesPage() {
  ...
  const remove = async (id: string) => {
    ...
    try { await AdminAPI.deleteCategory(id); await load(); toast.success("Deleted"); }
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="flex gap-2 flex-wrap">
          <Input placeholder="id" value={form.id} onChange={(e)=>setForm({...form,id:e.target.value})} className="w-40"/>
          <Input placeholder="name" value={form.name} onChange={(e)=>setForm({...form,name:e.target.value})} className="w-48"/>
          <Input placeholder="slug" value={form.slug} onChange={(e)=>setForm({...form,slug:e.target.value})} className="w-48"/>
