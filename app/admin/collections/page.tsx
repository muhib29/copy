    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="flex gap-2 flex-wrap">
          <Input placeholder="id (e.g. summer)" value={form.id} onChange={(e)=>setForm({...form,id:e.target.value})} className="w-48"/>
          <Select value={form.name} onValueChange={(v)=>setForm({...form,name:v as CollectionName})}>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={3}>No collections</TableCell></TableRow>
              ) : items.map((c)=> (
                <TableRow key={c.id}>
                  <TableCell>{c.id}</TableCell>
                  <TableCell>
                    {editId === c.id ? (
                      <Select defaultValue={c.name} onValueChange={(v)=>save(c.id, v as CollectionName)}>
                        <SelectTrigger className="w-48"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="Summer">Summer</SelectItem>
                          <SelectItem value="Winter">Winter</SelectItem>
                  ...
                  <TableCell className="text-right space-x-2">
                    <Button variant="outline" size="sm" onClick={()=>setEditId(editId===c.id?null:c.id)}>
