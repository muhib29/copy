...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState<ProductDTO>({ id: "", name: "", price: 0, image: "", collection: "Summer", category: "" });
  const [editForm, setEditForm] = useState<Omit<ProductDTO, "id">>({ name: "", price: 0, image: "", collection: "Summer", category: "" });
  ...
  const beginEdit = (p: ProductDTO) => {
    setEditId(p.id);
    setEditForm({ name: p.name, price: p.price, image: p.image, collection: p.collection, category: p.category });
  };

  const create = async () => {
    if (!form.id.trim() || !form.name.trim() || !form.image.trim() || !form.category.trim()) return toast.error("Fill all fields");
    setCreating(true);
    try {
      await AdminAPI.createProduct({ ...form, price: Number(form.price) });
      setForm({ id: "", name: "", price: 0, image: "", collection: form.collection, category: "" });
      await load();
  ...
  const save = async (id: string) => {
    try {
      await AdminAPI.updateProduct(id, { ...editForm, price: Number(editForm.price) });
      setEditId(null);
      await load();
  ...
  const remove = async (id: string) => {
    ...
    try { await AdminAPI.deleteProduct(id); await load(); toast.success("Deleted"); }
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid gap-2 md:grid-cols-6">
          ...
          <Select value={form.collection} onValueChange={(v)=>setForm({...form,collection:v as CollectionName})}>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={7}>No products</TableCell></TableRow>
              ) : items.map((p)=> (
                <TableRow key={p.id}>
                  <TableCell>{p.id}</TableCell>
                  <TableCell>{editId===p.id ? (
                  ...
                  <TableCell>{editId===p.id ? (
                    ...
                    <a href={p.image} target="_blank" rel="noreferrer" className="text-blue-600 underline">link</a>
                  )}</TableCell>
                  <TableCell>{editId===p.id ? (
                  ...
                  ) : p.collection}</TableCell>
                  <TableCell>{editId===p.id ? (
                    <Input value={editForm.category} onChange={(e)=>setEditForm({...editForm,category:e.target.value})} />
                  ) : p.category}</TableCell>
